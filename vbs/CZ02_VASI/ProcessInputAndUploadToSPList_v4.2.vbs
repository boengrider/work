...
'## 01.07.2022
'## BreakDown date missing in the input file handling
'##
'##
'#####################################################
...
Const S_SCCOVERAGE = "SCCoverage" ' <------------------> SCCOVERAGE
Const S_COMMENT = "Comment" ' <------------------------> ADDITIONAL COMMENTS
...
Const CRED_PATH = "C:\!AUTO\CREDENTIALS\logins.txt"
...
Dim s__brand, s__documentoutnumber, s__dealercountry, s__breakdowndate, s__doctype, s__sccoverage, s__comment
...
'@Load credentials
oXML.load CRED_PATH
oSP.Init oXML.selectSingleNode("//service[@name=""" & UNIT & """]/host").text, _
         oXML.selectSingleNode("//service[@name=""" & UNIT & """]/domain").text, _
         oXML.selectSingleNode("//service[@name=""" & UNIT & """]/username").text, _
         oXML.selectSingleNode("//service[@name=""" & UNIT & """]/password").text
...
For Each file In dictFilesInSourceDir.Keys
	...
	If oRX.Test(file) Then
		...
		Do While row <= numRows
			If Not oSP.ItemExists(SP_LIST_NAME,"select=Title&$filter=Title eq '" & Trim(oWS.Range(DOCUMENT_OUT_NUMBER & row).Value) & "' and DocType eq 'OUT'","d:Title") Then
				...
				'Check BreakdownDate. Sometimes they send reports without this value
				...
				If err.number <> 0 Then
					strBreakDate = "1/1/1980"
					s__comment = """" & S_COMMENT & """:""BreakdownDate missing or malformed"","
				Else
					s__comment = """" & S_COMMENT & """:"""","
				End If 
				On Error GoTo 0 
				...
				Else ' This VIN has a value in the contract file
					...
					debug.WriteLine " Contract starts:  " & cstart
					debug.WriteLine " Contract expires: " & cexp
					debug.WriteLine " Brakedown date:   " & bd
					If strBreakDate = "1/1/1980" Then
						'ctype = oWSC.Range("E" & oRNG.Row).Value
						ctype = "!"
						debug.WriteLine vbTab & ctype & " contract validity could not be verified. BreakdownDate missing"
						s__sccoverage = """" & S_SCCOVERAGE & """:""No"","
					ElseIf bd >= cstart And bd <= cexp Then
						ctype = oWSC.Range("E" & oRNG.Row).Value
						debug.WriteLine vbTab & ctype & " contract was valid at the time"
						s__sccoverage = """" & S_SCCOVERAGE & """:"""","
					Else
						debug.WriteLine vbTab & oWSC.Range("E" & oRNG.Row).Value & " contract was invalid at the time"
				...
				s__breakdowndate = """" & S_BREAKDOWN_DATE & """:""" & strBreakDate & ""","
				...
				s__dealerid = """" & S_DEALER_ID & """:""" & dictDealerLoc.Item(oRX.Replace(oWS.Range(DEALER_ID & row).Value,"")) & "-" & oRX.Replace(oWS.Range(DEALER_ID & row).Value,"") & ""","
				...
				ElseIf LCase(oWS.Range(BREAK_DOWN_PROD_LINE & row).Value) = "renault" Then
					s__brand = """" & S_BRAND & """:""RT"","
				End If
				s__nonvatchargeable = """" & S_NON_VAT_CHARGEABLE & """:""" & Replace(oWS.Range(NON_VAT_CHARGEABLE & row).Value,",",".") & """}" ' Ends the Json
				strRequest = s__pdf & s__documentoutnumber & s__requestnumber & s__contract & s__sccoverage & s__customername & s__currency & s__yearweek _ 
						   & s__vinserial & s__dealerid & s__docdate & s__totalamount & s__labour & s__regno & s__traveltime & s__brandedparts & s__dealercountry _
						   & s__deftempl & s__doctype & s__nonbrandedparts & s__envsundry & s__dealercallout & s__ccp & s__towingcost & s__servicevan & s__othercost _
						   & s__thirdpartywork & s__subtotal & s__chargedforeignvat & s__totaladmincharge & s__totalbelgianvat & s__breakdowndate & s__vatchargeable  _
						   & s__brand & s__comment & s__nonvatchargeable
		...
		row = 2
		Do While row <= numRows  
			If Not oSP.ItemExists(SP_LIST_NAME,"select=RequestNumber&$filter=RequestNumber eq '" & Trim(oWSCCCP.Range(REQUEST_NUMBER_CCP & row).Value) & "' and DocType eq 'CCP'","d:RequestNumber") Then
				...
				strDate = Month(CDate(oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value)) & "/" & Day(CDate(oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value)) & "/" & Year(CDate(oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value))
				'strDate = Month(CDate(oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value)) & "." & Day(CDate(oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value)) & "." & Year(CDate(oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value))
				debug.WriteLine "CCP ProcessedDate -> " & strDate
				debug.WriteLine "CCP CDate -> " & CDate(oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value)
				
				s__customername = """" & S_CUSTOMER_NAME & """:""" & Replace(oWSCCCP.Range(DEALER_NAME_CCP & row).Value,",","0") & ""","
				...
				s__dealerid = """" & S_DEALER_ID & """:""" & dictDealerLoc.Item(oRX.Replace(oWSCCCP.Range(DEALER_ID_CCP & row).Value,"")) & "-" & oRX.Replace(oWSCCCP.Range(DEALER_ID_CCP & row).Value,"") & ""","
				...
				s__breakdowndate = """" & S_BREAKDOWN_DATE & """:""" & strDate & ""","
				's__docdate = """" & S_DOC_DATE & """:""" & oWSCCCP.Range(PROCESSED_DATE_CCP & row).Value & ""","
				s__docdate = """" & S_DOC_DATE & """:""" & strDate & ""","
				s__documentoutnumber = """Title"":""" & oWSCCCP.Range(DOCUMENT_OUT_NUMBER_CCP & row).Value & ""","
				s__totalamount = """" & S_SUBTOTAL & """:""" & Replace(Replace(oWSCCCP.Range(TOTAL_AMOUNT_CCP & row).Value,",","."),"-","0") & """}"
				
				strRequest = s__pdf & s__brand & s__dealercountry & s__dealerid & s__customername & s__currency & s__doctype & s__requestnumber & s__breakdowndate & s__docdate & s__documentoutnumber & s__totalamount
				
				debug.WriteLine oSP.AddListItem("CZ02_VASI_Portal",strRequest)
			Else
				Debug.WriteLine Trim(oWSCCCP.Range(REQUEST_NUMBER_CCP & row).Value) & " already exists in the sharepoint list " & SP_LIST_NAME & ". Skipping this item"
			End If 

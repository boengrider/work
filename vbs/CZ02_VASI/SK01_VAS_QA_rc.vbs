...
' Other constants
Const outdir = "SK01_VASI" ' Output directory
Const spsourcedir = "SK01_VASI/SOURCE" ' Sharepoint online source directory
Const attachmentdir = "SK01_VASI/SK01_VASI_PDF" ' Sharepoint online attachments directory
Const SK_CONTRACTS = "SK01_VASI_Contracts.xlsx"
Const SP_LIST_NAME = "SK01_VAS_Invoicing_DEV"
'###########################################
'############## Variables ##################
'###########################################

'Sharepoint columns values
...
Dim s__brand, s__documentoutnumber, s__dealercountry, s__breakdowndate
...
Dim oSP,oHTTP,oFSO,oWSH,oFILE,oSTREAM,oXML,oCON,oRST,oEXCEL,oWB,oWS,oRX,oWBC,oWSC,oRNG
Set oRX = New RegExp 
...
'*******************************************************************
oSP.Init "https://volvogroup.sharepoint.com/sites/unit-rc-sk-bs-it","volvogroup.sharepoint.com","462ad7ed-2e5e-4175-b808-18c6f33fadd7","dWHEl4AMp8qHX/oxeFcY4RyFJJRD7z1cIavjDH53yIE="
'oSP.DeleteAllItemsInList "SK01_VAS_Invoicing_DEV" ' Delete all items in the list. Only in development phase
...
outpath = Left(oWSH.SpecialFolders("desktop"),1) & ":\!AUTO"
...
Next
If Not oFSO.FileExists(outpath & SK_CONTRACTS) Then
...
'*******************************************************************
Set oWBC = oEXCEL.Workbooks.Open(outpath & SK_CONTRACTS)
...
For Each file In dictFilesInSourceDir.Keys
	...
	If oRX.Test(file) Then
		...
		Do While row <= numRows
			If Not oSP.GetListItem(SP_LIST_NAME,"Title",Trim(oWS.Range(DOCUMENT_OUT_NUMBER & row).Value)) Then 
				...
				If UCase(Left(s__documentoutnumber,2)) = "IT" Then ' Volvo
					s__requestnumber = """" & S_REQUEST_NUMBER & """:{""__metadata"":{""type"":""SP.FieldUrlValue""},""Description"":""" & oWS.Range(REQUEST_NUMBER & row).Value & """,""Url"":""http://argus.srv.volvo.com/ARGUSTYCVOL"" }," ' Link to the TYCVOL
				ElseIf UCase(Left(s__documentoutnumber,2)) = "IZ" Then ' Renault
					 s__requestnumber = """" & S_REQUEST_NUMBER & """:{""__metadata"":{""type"":""SP.FieldUrlValue""},""Description"":""" & oWS.Range(REQUEST_NUMBER & row).Value & """,""Url"":""http://argus.srv.volvo.com/ARGUSTYCREN"" }," ' Link to the TYCREN
				End If 
				...
				Else ' This VIN has a value in the contract file
					...
					If bd >= cstart And bd <= cexp Then
						...
						debug.WriteLine "    " & ctype & " contract was valid"
					Else
						debug.WriteLine "    " & ctype & " contract was invalid"
						ctype = "-" ' Contract was not valid at the time
					End If 
				End If 
					debug.WriteLine "    Breakdown date -> " & bd
				...
				Select Case ctype
					...
					Case "-"
						ctype = "-" ' No contract was found or contract was not valid at the time
					...
					Case Else
						ctype = "?" ' Something else. Uknown contract
				...
				s__totalamount = """" & S_TOTAL_AMOUNT & """:""" & Replace(oWS.Range(TOTAL_AMOUNT & row).Value,",",".") & ""","
				s__labour = """" & S_LABOUR & """:""" & Replace(oWS.Range(LABOUR & row).Value,",",".") & ""","
				s__traveltime = """" & S_TRAVEL_TIME & """:""" & Replace(oWS.Range(TRAVEL_TIME & row).Value,",",".") & ""","
				s__brandedparts = """" & S_BRANDED_PARTS & """:""" & Replace(oWS.Range(BRANDED_PARTS & row).Value,",",".") & ""","
				s__dealercountry = """" & S_DEALER_COUNTRY & """:""" & oWS.Range(DEALER_COUNTRY & row).Value & ""","
				s__nonbrandedparts = """" & S_NON_BRANDED_PARTS & """:""" & Replace(oWS.Range(NON_BRANDED_PARTS & row).Value,",",".") & ""","
				s__envsundry = """" & S_ENV_SUNDRY & """:""" & Replace(oWS.Range(ENV_SUNDRY & row).Value,",",".") & ""","
				s__dealercallout = """" & S_DEALER_CALLOUT & """:""" & Replace(oWS.Range(DEALER_CALLOUT & row).Value,",",".") & ""","
				s__ccp = """" & S_CCP & """:""" & Replace(oWS.Range(CCP & row).Value,",",".") & ""","
				s__towingcost = """" & S_TOWING_COST & """:""" & Replace(oWS.Range(TOWING_COST & row).Value,",",".") & ""","
				s__servicevan = """" & S_SERVICE_VAN & """:""" & Replace(oWS.Range(SERVICE_VAN & row).Value,",",".") & ""","
				s__othercost = """" & S_OTHER_COST & """:""" & Replace(oWS.Range(OTHER_COST & row).Value,",",".") & ""","
				s__thirdpartywork = """" & S_THIRD_PARTY_WORK & """:""" & Replace(oWS.Range(THIRD_PARTY_WORK & row).Value,",",".") & ""","
				s__subtotal = """" & S_SUBTOTAL & """:""" & Replace(oWS.Range(SUBTOTAL & row).Value,",",".") & ""","
				s__chargedforeignvat = """" & S_CHARGED_FOREIGN_VAT & """:""" & Replace(oWS.Range(CHARGED_FOREIGN_VAT & row).Value,",",".") & ""","
				s__totaladmincharge = """" & S_TOTAL_ADMIN_CHARGE & """:""" & Replace(oWS.Range(TOTAL_ADMIN_CHARGE & row).Value,",",".") & ""","
				s__totalbelgianvat = """" & S_TOTAL_BELGIAN_VAT & """:""" & Replace(oWS.Range(TOTAL_BELGIAN_VAT & row).Value,",",".") & ""","
				s__vatchargeable = """" & S_VAT_CHARGEABLE & """:""" & Replace(oWS.Range(VAT_CHARGEABLE & row).Value,",",".") & ""","
				...
				strRequest = s__pdf & s__documentoutnumber & s__requestnumber & s__contract & s__customername & s__currency & s__yearweek & s__vinserial & s__dealerid _
						   & s__docdate & s__totalamount & s__labour & s__regno & s__traveltime & s__brandedparts & s__dealercountry & s__deftempl _
				...
				debug.WriteLine oSP.AddListItem("SK01_VAS_Invoicing_DEV",strRequest)
				debug.WriteLine
	...
	End If 
Next
debug.WriteLine "Closing contracts file"
oWBC.Close
'**************** MOVE ******************************
'oSP.MoveFile2 "SK01_VASI/SOURCE/PROCESSED/SLOVAKIA_2020W46.xlsx","SK01_VASI/SOURCE/SLOVAKIA_2020W46.xlsx"
'***************** DELETE ***************************

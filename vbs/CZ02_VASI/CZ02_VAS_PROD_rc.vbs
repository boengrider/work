...
For Each file In dictFilesInSourceDir.Keys
	...
	If oRX.Test(file) Then
		...
		row = 2
		Do While row <= numRows
			If Not oSP.GetListItem(SP_LIST_NAME,"RequestNumber",Trim(oWSCCCP.Range(REQUEST_NUMBER_CCP & row).Value)) Then 
				...
				s__breakdowndate = """" & S_BREAKDOWN_DATE & """:""" & oWSCCCP.Range(BREAKDOWN_DATE_CCP & row).Value & ""","

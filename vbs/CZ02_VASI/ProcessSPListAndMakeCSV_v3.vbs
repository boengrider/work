...
If oMyItems.CCPCount + oMyItems.OUTCount > 0 Then
	...
	Dim vasiItems : Set vasiItems = oMyItems.ItemsCCP
	For Each itemNode In vasiItems.Keys
		f.Write vasiItems.Item(itemNode).GetHeader & vasiItems.Item(itemNode).GetLineItems
'		debug.Write vasiItems.Item(itemNode).GetHeader
'		debug.Write vasiItems.Item(itemNode).GetLineItems
	Next
...
'			debug.Write vasiItems.Item(itemNode).GetLineItems
		Else
			debug.WriteLine "Invoice " & vasiItems.Item(itemNode).GetInvoice & " not recognized"
		End If 
	Next
	...
	Public Function Consume(xmlNode__,xmlConfigNode__)
		...
		Select Case UCase(xmlNode__.SelectSingleNode("content").selectSingleNode("m:properties").selectSingleNode("d:DocType").text)
			...
			Case "CCP"
				...
				'listItemsCCP__.Item(invoice__).OK = True This is not required since CCP items do not have special conditions for GL lines
			...
			Case "OUT"
				...
				'***********************************
				'GL Line1 may or may not be present.
				...
				'***********************************
				'GL Line2 lines must be in order
				'***********************************

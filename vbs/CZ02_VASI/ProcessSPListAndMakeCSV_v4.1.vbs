...
If oMyItems.CCPCount + oMyItems.OUTCount > 0 Then
	outFile = "CZ02_A7_AP-SK01-D_VASI_2d_" & oDF.ToYearMonthDayHourMinuteWithZeros(Date,Time) & ".csv"
	Dim f : Set f = oFSO.OpenTextFile(oXMLCONF.selectSingleNode("//WorkingDirectory").text & outFile, 2, True)
...
For Each itemNode In vasiItems.Keys
	debug.WriteLine "Item of type '" & vasiItems.Item(itemNode).GetDocType & "' is OK -> " & CStr(vasiItems.Item(itemNode).IsOK)
	If vasiItems.Item(itemNode).IsOK Then
		For Each id In vasiItems.Item(itemNode).GetIds
			debug.WriteLine "  Patching record with ID: " & id
...
Else 'Initialization OK
	token = oSPL.AccessToken
	xdigestvalue = oSPL.XDigest
	oSPL.UploadSingleFile Null, LIBRARY_DEST, oXMLCONF.selectSingleNode("//WorkingDirectory").text & outFile, True
End If
	...
	Public Function Consume(xmlNode__,xmlConfigNode__)
		...
		Select Case UCase(xmlNode__.SelectSingleNode("content").selectSingleNode("m:properties").selectSingleNode("d:DocType").text)
			...
			Case "CCP"
				...
				'CostCenter -> line item (11) if config node true, use matrix else universal cc
				If CBool(xmlConfigNode__.SelectSingleNode("CaseCCP").SelectSingleNode("DealerIdCC").text) Then
					listItemsCCP__.Item(invoice__).LineItemCostCenter = xmlConfigNode__.SelectSingleNode("DealerIdMatrix").SelectSingleNode(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:DealerId").text).attributes.getNamedItem("CC").text
				Else
			...
			Case "OUT"
				...
				If (CDbl(Replace(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:TotalAdminCharge").text,".",",")) > 0 And _
				   ...
				   UCase(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:DefTempl").text) = "NO") Then
					...
					'TotalAmount -> line item (8)
					listItemsOUT__.Item(invoice__).LineItemTotalAmount = xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:TotalAdminCharge").text
					'TaxCode -> line item (9)
				...
				'**************************************************************************************
				' (1) TotalAdminCharge = 0 & SubTotal = 0 & ChargedForeignVAT = 0 & NonVATChargable > 0
				'**************************************************************************************
				If CDbl(Replace(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:TotalAdminCharge").text,".",",")) = 0 And _
				   ...
				   CDbl(Replace(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:NonVATChargeable").text,".",",")) > 0 Then
				    '***************************************
				   	...
				   	If UCase(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:SCCoverage").text) = "NO" Then
						...
						listItemsOUT__.Item(invoice__).LineItemSpecialGL = ""
						'TotalAmount -> line item (8)
						listItemsOUT__.Item(invoice__).LineItemTotalAmount = xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:NonVATChargeable").text
						'TaxCode -> line item (9)
						...
						listItemsOUT__.Item(invoice__).LineItemTaxAmount = "0,00"
						'CostCenter -> line item (11)
						listItemsOUT__.Item(invoice__).LineItemCostCenter = xmlConfigNode__.SelectSingleNode("CaseOUT").SelectSingleNode("VAS1").SelectSingleNode("CC").text
						'ProfitCenter -> line item (12)
					...
					'****************************************
				   	...
				   	ElseIf UCase(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:SCCoverage").text) = "FULL" Then
						...
						'ProfitCenter -> line item (12)
						'If DealerIdPC true then PC comes from matrix else i don't know
				   	...
				   	ElseIf UCase(xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:SCCoverage").text) = "PARTIAL" Then
						...
						listItemsOUT__.Item(invoice__).LineItemSpecialGL = ""
						'TotalAmount -> line item (8)
						listItemsOUT__.Item(invoice__).LineItemTotalAmount = xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:SCCoverageAmount").text
						'TaxCode -> line item (9)
						listItemsOUT__.Item(invoice__).LineItemTaxCode = xmlConfigNode__.SelectSingleNode("CaseOUT").SelectSingleNode("VAS1").SelectSingleNode("TaxCode1").text
						'TaxAmount -> line item (10)
						listItemsOUT__.Item(invoice__).LineItemTaxAmount = "0,00"
				...
				ElseIf CBool(xmlConfigNode__.SelectSingleNode("//AdminChargeSeparate").text) = False And _
			       ...
			       '**************************************************************************************
				   ' (2) Config.AdminChargeSeparate = False & DefTempl = YES
				   '**************************************************************************************
					'DocDate -> line item (1)
					listItemsOUT__.Item(invoice__).LineItemDocDate = xmlNode__.SelectSingleNode("content").SelectSingleNode("m:properties").SelectSingleNode("d:DocDate").text
					'PostingDate -> line item (2)
